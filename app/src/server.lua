...
local function httpparse(data)
  local res = {}
  res.status = decodeURL(data:sub(0, data:find('\r\n')))
...
function server_start()
  ...
  local function recv_cb(conn, data)
    ...
    print('PARSED STATUS:  ', httpdata.status)
    ...
    if qs.ssid and qs.pwd then
      local wifi_config = { ssid=qs.ssid, pwd=qs.pwd, save=true }
      wifi.sta.config(wifi_config)
      print('connecting to wifi...     PASSWORD = ', qs.pwd)
      conn:send(
    ...
    -- the raw state at the DEV.state
